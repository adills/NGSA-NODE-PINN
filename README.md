# Hybrid NSGA-PINN and NSGA-NODE Framework

[![Python 3.12+](https://img.shields.io/badge/python-3.12%2B-blue.svg)](https://www.python.org/downloads/)
[![uv](https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/astral-sh/uv/main/assets/badge/v0.json)](https://github.com/astral-sh/uv)
[![PyTorch](https://img.shields.io/badge/PyTorch-2.0%2B-ee4c2c.svg)](https://pytorch.org/)

A robust, modular framework for **Multi-Objective Evolutionary Scientific Machine Learning**. This project implements the **Hybrid NSGA-PINN** and **NSGA-NODE** architectures, combining the local refinement capabilities of gradient-based optimization (ADAM) with the global search and multi-objective handling of evolutionary algorithms (NSGA-II). The NSGA-PINN theory is documented in NSGA-PINN paper but it has no promised code.[1](https://www.mdpi.com/1999-4893/16/4/194).  

[1](https://www.mdpi.com/1999-4893/16/4/194) B. Lu, C. Moya, and G. Lin, NSGA-PINN: A Multi-Objective Optimization Method for Physics-Informed Neural Network Training, Algorithms 16, 194 (2023).

## AMOS Conference Abstract (Draft)

**Title:** Hybrid NSGA-PINN and NSGA-NODE for Multi-Objective Scientific Machine Learning

**Abstract:** Physics-informed learning is often framed as a single-objective optimization problem, where data misfit and physics residuals are combined with user-chosen weights. While practical, scalarization can obscure the real trade-offs between fidelity to measurements and adherence to governing equations, and it can be fragile when loss magnitudes drift or the dynamics are stiff. We present a hybrid multi-objective framework that treats these competing goals explicitly and solves them with the Non-dominated Sorting Genetic Algorithm II (NSGA-II) [2]. Building on NSGA-PINN [1], our approach evolves a population of candidate networks to approximate the Pareto front while alternating with standard gradient-based refinement (ADAM) for rapid local convergence. The result is a principled, data–physics trade-space that can be explored and selected according to mission-specific constraints, rather than an arbitrary tuning of loss weights.

This paper introduces the NSGA-NODE extension for systems that require improved or partially unknown ordinary differential equation (ODE) models. Neural ODEs provide a natural representation for continuous-time dynamics, but their training is typically single-objective and often sensitive to initialization and solver choices. We define NSGA-NODE as a hybrid evolutionary–gradient framework in which (i) the data misfit over observed trajectories and (ii) a correction magnitude objective (the norm of the learned correction to the physics model) are optimized simultaneously. The second objective encourages parsimony: it biases solutions toward physics-consistent dynamics and only uses the neural correction where evidence requires it. This “physics-first” regularization emerges from the Pareto front rather than from a fixed hyperparameter.

A central technical contribution is population-level evaluation of neural ODEs with batched parameter sets. In NSGA-PINN, vectorized residual evaluation is naturally parallelized with `torch.func` and `vmap`. For NSGA-NODE, however, each individual in the population carries a distinct set of ODE parameters, which makes naive ODE solves prohibitively expensive. We address this with a batched, parameterized ODE solve that allows each candidate’s dynamics to be evaluated concurrently, enabling practical population sizes on CPU/GPU. This unlocks a scalable evolutionary search over ODE models while preserving the differentiability required for ADAM refinement between NSGA-II generations.

We demonstrate the framework with two canonical examples included in this repository: (1) Burgers’ equation for NSGA-PINN, and (2) a damped oscillator for NSGA-NODE. These examples illustrate how Pareto fronts reveal trade-offs between data fit and physics consistency, and how hybrid handoff between NSGA-II and ADAM accelerates convergence while avoiding common local minima. Beyond these test cases, the approach generalizes to any physics-based analysis in which governing equations are partially known, uncertain, or require correction—such as orbital dynamics, atmospheric reentry, sensor fusion for space domain awareness, and other systems where model fidelity and data consistency must be balanced explicitly.

The outcome is a multi-objective scientific machine learning workflow that is both explainable and operational. By exposing the full Pareto front and providing a consistent handoff protocol between evolutionary search and gradient descent, NSGA-PINN/NSGA-NODE offers a robust path to model discovery and uncertainty-aware decision-making. We anticipate this approach will be valuable for AMOS applications where physical models are trusted but incomplete, and where mission risk depends on transparent trade-offs between model fidelity and empirical fit.

**Summary of Presentation (AMOS, 250 words or less):** This presentation introduces a hybrid multi-objective training workflow for physics-informed neural networks (NSGA-PINN) and neural ODEs (NSGA-NODE). We show how NSGA-II can replace ad hoc loss weighting by exposing a Pareto front between data fit and physics consistency, then use ADAM for local refinement. For NSGA-NODE, we demonstrate a population-based evaluation that batches parameterized dynamics in a parallel ODE solver, enabling practical evolutionary search over ODE corrections. The talk walks through two repository examples (Burgers' equation and a damped oscillator), highlighting how the correction-magnitude objective drives parsimonious physics updates and how the handoff protocol stabilizes training. Attendees will see how to interpret Pareto fronts, select knee-point models, and quantify terminal state errors. We conclude with guidance for extending the method to space-domain dynamics and other partially-known systems where model fidelity and data consistency must be balanced. We emphasize relevance to SSA/SDA by enabling transparent trade-offs for trajectory prediction and model correction under uncertainty.

**Keywords:** NSGA-II, NSGA-PINN, NSGA-NODE, multi-objective optimization, physics-informed learning, neural ODEs, scientific machine learning

**References:**  
[1](https://www.mdpi.com/1999-4893/16/4/194) B. Lu, C. Moya, and G. Lin, NSGA-PINN: A Multi-Objective Optimization Method for Physics-Informed Neural Network Training, Algorithms 16, 194 (2023).  
[2](https://ieeexplore.ieee.org/document/996017) K. Deb, A. Pratap, S. Agarwal, and T. Meyarivan, A fast and elitist multiobjective genetic algorithm: NSGA-II, IEEE Trans. Evol. Comput. 6(2):182–197 (2002).

## Alternate Abstract (Undergraduate Physics Level)

**Abstract:** In many physics problems we want a model that both matches measurements and respects known laws. A common trick is to add the “data error” and “physics error” together with a chosen weight. That can work, but the right weight is often unclear, and the solution can change a lot when the weight changes. In this project we treat those two errors as separate goals and use a multi‑objective algorithm (NSGA‑II) to find a set of best trade‑offs, called a Pareto front.

We apply this idea in two settings. First, NSGA‑PINN trains a physics‑informed neural network so that one objective is data fit and the other is physics consistency. Second, we extend the same idea to neural ODEs (NSGA‑NODE), where a neural network corrects or augments an ODE model. For NSGA‑NODE we also add a “correction size” objective so the learned correction stays small unless the data strongly requires it. This makes the model prefer physics‑consistent solutions whenever possible.

A technical challenge is that NSGA‑II evaluates many candidate networks at once. For neural ODEs, each candidate has different parameters, so solving the ODE separately for each one would be too slow. We address this by batching ODE solves so the whole population can be evaluated together on CPU/GPU. The repository includes two simple examples—Burgers’ equation and a damped oscillator—that show how the Pareto front reveals the trade‑off between data and physics. The same approach can be applied to more complex systems where the equations are only partially known.

## Motivation

The primary objective is to operationalize the NSGA-PINN framework, as conceptualized in recent literature (e.g., [1] MDPI 2023), which fundamentally redefines the training of scientific machine learning models. Instead of the traditional scalarization approach—where data loss and physics loss are combined into a weighted sum—this framework treats them as competing objectives to be optimized simultaneously via the Non-dominated Sorting Genetic Algorithm II (NSGA-II). [bibliography](docs/bibliography.md)

The motivation for this architectural shift lies in the limitations of gradient-based optimizers in the context of scientific machine learning. While Stochastic Gradient Descent (SGD) and ADAM are highly effective for convex or smooth loss landscapes, they frequently encounter severe pathologies in the training of PINNs, including spectral bias, entrapment in suboptimal local minima, and sensitivity to hyperparameter tuning of loss weights. By employing an evolutionary approach, specifically NSGA-II, the proposed system leverages a global search capability that generates a Pareto front of optimal solutions. This allows researchers to rigorously analyze the trade-offs between adherence to physical laws (physics loss) and fidelity to empirical measurements (data loss) without the arbitrary selection of weighting coefficients.

Following an exhaustive evaluation of the Python evolutionary computation ecosystem—specifically analyzing `PyGAD`, `DEAP`, and `pymoo`—the evidence advocates for a hybrid architectural approach. We recommend utilizing `pymoo` as the evolutionary orchestration engine due to its rigorous object-oriented design, superior handling of multi-objective constraints, and native support for vectorized problem definitions. This is coupled with **PyTorch’s Functional API (**<code>torch.func</code>**)** to manage the massive throughput required for evaluating populations of neural networks on hardware accelerators (GPUs).

Furthermore, the project addresses the specific extension of this framework to Neural ODEs (NSGA-NODE). It identifies a critical computational bottleneck in standard ODE solvers (`torchdiffeq`) when applied to evolutionary populations: the inability to efficiently batch-solve differential equations where the dynamics parameters vary across the batch. To resolve this, the blueprint proposes the integration of `torchode`, a parallel ODE solver library capable of independent batch-parameter handling. This integration is essential for scaling the NSGA-NODE approach, ensuring that the evolutionary evaluation remains computationally feasible.

The project uses the class-based design of the module, encompassing the `NeuroEvolutionOrchestrator`, `PytorchGenomeInterface`, and `VectorizedEvaluator`. It provides a capability for implementation, emphasizing performance optimization through JIT compilation and vectorization, ultimately delivering a robust tool for advanced scientific discovery.

**Current status:**
* [NGSA-PINN development tasks](docs/NSGA_PINN_tasks.md)
    * Completed, Verified with Burgers' Equation.
    * See [verify_hybrid_pinn.py](examples/verify_hybrid_pinn.py) for details.
* [NSGA-NODE development tasks](docs/NSGA_NODE_tasks.md)
    * TODO: Verify with Damped Oscillator.
    * TODO: [verify_node_oscillator.py](examples/verify_node_oscillator.py) for details.
 


## 🎯 Project Goals

1.  **Escape Local Minima:** Use NSGA-II to explore the parameter landscape and find better initialization basins for PINNs and Neural ODEs.
2.  **Pareto Optimality:** Explicitly trade off **Data Fitting** vs. **Physical Consistency** (or Model Parsimony) without manual loss weighting.
3.  **Hybrid Training:** seamless "handoff" protocols between evolutionary search (gradient-free/input-grad only) and standard gradient descent (ADAM).
4.  **High Performance:** Leverage `torch.func` (vmap) and `torchode` to parallelize evaluations across entire populations on GPU/CPU.

## 🛠️ Tech Stack & Prerequisites

-   **Python:** 3.12+
-   **Package Manager:** [uv](https://github.com/astral-sh/uv) (Extremely fast, modern Python package installer)
-   **Core Libraries:**
    -   `pytorch`: Deep learning and Autograd.
    -   `pymoo`: Multi-objective optimization algorithms.
    -   `torchode`: Parallel ODE solvers for Neural ODEs.
    -   `numpy`: Numerical operations.

## 📦 Installation

This project uses `uv` for dependency management.

1.  **Install uv** (if not already installed):
    ```bash
    # MacOS / Linux
    curl -LsSf https://astral.sh/uv/install.sh | sh

    # Windows
    powershell -c "irm https://astral.sh/uv/install.ps1 | iex"
    ```

2.  **Clone and Sync:**
    ```bash
    git clone <repository_url>
    cd <repository_name>

    # Create virtualenv and install dependencies
    uv sync
    ```

## ⚡ Quick Start

### 1. Validate PINN Module (Burgers' Equation)
Run the hybrid training verification script for the Physics-Informed Neural Network:

```bash
uv run examples/verify_hybrid_pinn.py --device cpu
# Or on GPU
uv run examples/verify_hybrid_pinn.py --device cuda
```

### 2. Validate NODE Module (Damped Oscillator)
Run the hybrid training verification script for the Neural ODE:

```bash
uv run examples/verify_node_oscillator.py --device cpu
```

## 🔌 Third-Party Baselines (Optional)

This repo includes third-party apps under `third_party/` for validation-only comparisons. These are not required for core development.

### Included app: `third_party/pinn_burgers`

Purpose: provides a baseline PINN implementation for Burgers' Equation so you can compare Hybrid NSGA-PINN against a standard PINN.

Install/use:
1. Clone the baseline into the repo:
   ```bash
   git clone https://github.com/okada39/pinn_burgers third_party/pinn_burgers
   ```
2. Run the comparison:
   ```bash
   uv run examples/verify_hybrid_pinn.py --compare_third_party
   ```

## 📂 Project Structure

```text
.
├── src/
│   ├── nsga_neuro_evolution_core/  # Shared utilities (Genome interface, Gradient contexts)
│   ├── nsga_pinn/                  # NSGA-PINN specific logic (Evaluator, Problem, Orchestrator)
│   └── nsga_node/                  # NSGA-NODE specific logic (Dynamics, Torchode wrapper)
├── tests/
│   ├── core/                       # Unit tests for shared modules
│   ├── pinn/                       # Unit tests for PINN
│   └── node/                       # Unit tests for NODE
├── docs/
│   ├── NSGA_PINN_tasks.md          # Detailed development roadmap for PINN
│   └── NSGA_NODE_tasks.md          # Detailed development roadmap for NODE
├── examples/                       # Validation scripts and demos
├── pyproject.toml                  # Project configuration and dependencies
└── uv.lock                         # Lock file for reproducible builds
```

## 🔬 Methodology

### The Hybrid Loop
This framework implements a specific contract to alternate between optimization modes:

1.  **ADAM Phase (Epochs):**
    -   Standard Gradient Descent.
    -   **Mode:** `gradient` (Full Autograd).
    -   Optimizes weights locally.

2.  **NSGA-II Phase (Generations):**
    -   Evolutionary Strategy.
    -   **Mode:** `fitness` (Weights Frozen, Input Grads enabled for Residuals).
    -   Explores global landscape using a population centered on the current ADAM solution.

3.  **Handoff Protocol:**
    -   Select best "Knee Point" or balanced individual from the Pareto Front.
    -   **Reset** optimizer state (momentum buffers) to avoid stale history.
    -   Initialize next ADAM phase with the evolved weights.

### Performance Optimizations
-   **PINN Vectorization:** Uses `torch.vmap` and `torch.func.functional_call` to evaluate hundreds of networks in a single forward pass.
-   **NODE Batching:** Uses `torchode` with batched parameters to solve differential equations for the entire population in parallel.

## 📚 Documentation
Detailed development task lists and architectural blueprints can be found in the `docs/` directory:
-   [NSGA-PINN Roadmap](docs/NSGA_PINN_tasks.md)
-   [NSGA-NODE Roadmap](docs/NSGA_NODE_tasks.md)
-   [bibliography](docs/bibliography.md)

## 📄 License
[MIT License](LICENSE)
